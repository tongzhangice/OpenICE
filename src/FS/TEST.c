...
static double **data_bot, **data_sur, **data_thk, **data_mask, **data_x, **data_y;
...
//    SIMPLEX *e;
    ...
    data_mask = read_txt_data(ns_params->mask_txt_file);
...
//	FILE *fp1, *fp2;
	int i, j, I, J;
	double bed, sur, thk, bot, mask;
    FLOAT Bx, By, B0, B2, B4, B6, x_bar, dc, Ly, wc, fc, Bmax;
    //double **data_bed, **data_sur, **data_thk;
    x *= 1000;
    y *= 1000;    

    B0 = -150.0; B2 = -728.8; B4 = 343.91; B6 = -50.57;
    x_bar = 3.0e5;
    dc = 500.0; Ly = 8.0e4; wc = 2.4e4; fc = 4.0e3;
    Bmax = 720.0;

    Bx = B0 + B2*pow(x/x_bar, 2) + B4*pow(x/x_bar, 4) + B6*pow(x/x_bar, 6);
    By = dc/(1+exp(-2*(y-Ly/2.0-wc)/fc)) + dc/(1+exp(2*(y-Ly/2.0+wc)/fc));
    ...
    //printf("bot, thk: %lf %lf, %f, %f\n", bot, thk, x, y);

    for (i = 0; i < 114; i++)
        if (fabs(x-data_x[0][i]) < 1e-2)
        {
            I = i;
            break;
        }
...
#   warning BD mask set to (x,x,x)
    BTYPE DB_masks[3] = {BC_BOTTOM|BC_LATERL|BC_DIVIDE|BC_TERMNS,
			 BC_BOTTOM|BC_LATERL,
			 BC_BOTTOM|BC_LATERL};
#else
