/* this is for testing an ice cubic with flat surface under 
 * hydrostatic stresses from its surroundings
 * if we don't prescribe Dirichlet BCs like u = 0 at the boundary,
 * will the cubic stay stable?
 */
...
func_sur_grad_y(FLOAT x, FLOAT y, FLOAT z, FLOAT *sur_grad_y)
{
    ...
    sur_grad_y[0] = 0; // ds/dy = 0
}
void
func_ice_sur(FLOAT x, FLOAT y, FLOAT z, FLOAT *ice_sur)
{
    ice_sur[0] = 0; // the surface elevation is 0
}
void
